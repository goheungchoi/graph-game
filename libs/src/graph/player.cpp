...
int Player::action(Graph &g, int layer, int index) {
  ...
  if (selectedNode == nullptr) {
    std::cerr << "Player::action: selected node is null" << std::endl;
    return _FAIL;
  }
  // 2. not adjancent to any user nodes
  if (selectedNode->color != _YELLOW) {
    std::cerr << "Player::action: selected node is not marked as adjacent node" << std::endl;
    std::cerr << "Player::action: the selected node is (" 
    << selectedNode->pos[0] << ", " << selectedNode->pos[1] << ")" << std::endl;
    return _FAIL;
  }
...
std::vector<Node*>& Player::getSelectedNodes() {
  return selectedNodes;
}

std::vector<Edge*>& Player::getPlayerPath() {
  return playerPath;
}
