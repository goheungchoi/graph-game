...
public:
   ...
   */
  bool local_traversal(Node* curr, bool& closed);
  /**
  ...
  std::vector<Node*>& getNewConsumedNodes() {
    return newConsumedNodes;
  }

  std::vector<Edge*>& getNewEntityPath() {
    return newEntityPath;
  }

  std::vector<Node*>& getConsumedNodes() {
    return consumedNodes;
  }

  std::vector<Edge*>& getEntityPath() {
    return entityPath;
  }
private:
  Node* origin = nullptr;
